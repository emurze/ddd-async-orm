...
from seedwork.container.middlewares import event_collector_middleware, \
    error_handling_middleware
from seedwork.container.provider import ContainerProvider
...
def create_application(config, db_engine) -> Application:
    ...
    def on_enter_transaction_context(ctx: TransactionContext) -> None:
        ...
        ctx.set_dependencies(publish=message_outbox.publish)
    ...
    async def on_exit_transaction_context(
        ...
        if exception or ctx.dependency_provider.has_dependency("is_error"):
